...
- Latest search cleanup: fuzzy typo matching now only runs after exact/intent matching finds no sponsors, so `lumber` returns only McCauley Lumber instead of fuzzy-matching `plumber`. Added Abacus attic insulation, attic ventilation, solar attic fan, and attic tent terms from the reviewed site text so `attic`, `insulation`, and `attic insulation` produce relevant results. Screenshot evidence: `/tmp/mb-show-lumber-clean-results.png`, `/tmp/mb-show-attic-insulation-results.png`.
