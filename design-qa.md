...
- Latest intelligent search pass: replaced raw literal substring matching with whole-word phrase matching, listener-intent aliases, weighted sponsor fields, and typo-tolerant matching only for terms the listener actually typed. `tree` now returns Ability Tree Service, ABC Home and Commercial Pest Control, and Brookway Landscape and Irrigation without MainStreet Wealth Management. `who can fix my air conditioner` returns the HVAC sponsors, and misspelled `pluming` returns Abacus only. Screenshot evidence: `/tmp/mb-show-intelligent-tree-results-full.png`, `/tmp/mb-show-intelligent-hvac-results-full.png`.
- Latest search cleanup: fuzzy typo matching now only runs after exact/intent matching finds no sponsors, so `lumber` returns only McCauley Lumber instead of fuzzy-matching `plumber`. Removed the Abacus attic/insulation terms because they made a plumbing company appear for insulation searches; `insulation`, `attic`, and `attic insulation` now infer home remodeling, siding/exterior, roofing, building-material, builder, and truss sponsors instead. Screenshot evidence: `/tmp/mb-show-lumber-clean-results.png`, `/tmp/mb-show-insulation-remodeling-results.png`.
- Search relevance hardening: removed broad edit-distance fuzzy matching, replaced it with a small typo-correction list, and changed inferred searches to a related-sponsors fallback that only runs for curated intent phrases. `retaining` no longer matches retail-adjacent or similar-looking words; it returns Daniel Dean, Cowboy Topsoil, and Brookway only, with unrelated fence, coffee, classic car, cleaning, and floor coating sponsors excluded by regression tests. Screenshot evidence: `/tmp/mb-show-retaining-curated-results.png`.
...
- Compact directory update: changed the sponsor directory from hidden/search-first results to a full 80-sponsor compact card list on initial load. Each card shows sponsor name, phone, and website, expands inline for description/category/services/service area, and the search input now filters the visible list in place. Screenshot evidence: `/tmp/mb-show-directory-compact-final-desktop.png`, `/tmp/mb-show-directory-expanded-final-desktop.png`, `/tmp/mb-show-directory-mobile-section-final.png`, `/tmp/mb-show-directory-mobile-expanded-final.png`, `/tmp/mb-show-directory-filtered-final-desktop.png`.
- CST text update: centered each red schedule-card time and moved plain `CST` text below the time on desktop and mobile. Screenshot evidence: `/tmp/mb-show-cst-text-desktop.png`, `/tmp/mb-show-cst-text-mobile.png`.
